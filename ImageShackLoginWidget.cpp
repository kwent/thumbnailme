...
{
    return loginAccountLineEdit->text().toLatin1().toBase64();
}
...
{
    return passwordAccountLineEdit->text().toLatin1().toBase64();
}
...
QNetworkProxy *ImageShackLoginWidget::readProxySettings()
{
  ...
  if (settings->value("Proxy/enabled").toBool())
  {
    ...
    {
      qDebug() << QByteArray::fromBase64(settings->value("Proxy/username").toByteArray());
