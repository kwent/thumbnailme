...
SuccessDialog::SuccessDialog(QWidget *parent, QString openFolderPath) : QDialog(parent)
{
    ...
    QVBoxLayout *mainLayout = new QVBoxLayout;
        ...
        mainLayout->addWidget(QtHelper::addHSeparator());
...
int SuccessDialog::addItem(ThumbnailItem *item)
{
    ...
    */
    ...
    i->setText(item->getFilePathOutput().toString(QUrl::RemoveScheme).mid(3));
    i->setParent(item);
...
void SuccessDialog::uploadToImageShack()
{
    ...
    imageShackUploadWidget->addData(QtHelper::getItemsListWidget(listWidget,Qt::Checked));
    this->close();
    QtHelper::centerWidget(imageShackUploadWidget);
...
void SuccessDialog::enableUploadButton()
{
    ...
    if(QtHelper::getItemsListWidget(listWidget,Qt::Checked).isEmpty())
...
*/
int SuccessDialog::exec()
{
    if (QtHelper::getItemsListWidget(listWidget).count() == 1)
    {
