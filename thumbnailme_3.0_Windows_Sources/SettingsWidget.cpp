...
{
    if(readPathsFontsValueIni(settings) != getItemsListWidget(fontsTab->getFontPathListWidget()))
    {
        updatePathsFontsIni();
...
void SettingsWidget::updateFontsListEngine()
{
    ...
    QMap <QString,QString> mapGlobal = exploreTTFDir(getItemsListWidget(fontsTab->getFontPathListWidget()));
...
void SettingsWidget::updateOpenGlGraphicViewIni(bool checked)
{
    ...
    QFlags<QGLFormat::OpenGLVersionFlags> openGL_versions (QGLFormat::openGLVersionFlags ());
...
{
    settings->setValue("Extras/backgroundColorPreviewWindow",qColor2rgbNoSharp(color));
